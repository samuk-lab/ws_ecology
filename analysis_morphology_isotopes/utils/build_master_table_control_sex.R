...
sl_fits <- pheno_long_sl %>% 
  select(-id, -sex, -geno_sex, -year, -region, -cluster, -sequenced) %>%
  filter(!grepl("raker|test|egg|plate", trait)) %>%
  mutate(joint_sex = as.factor(joint_sex)) %>%
  .[complete.cases(.),] %>%  
  group_by(trait) %>%
  do(fit_lm = anova(lm(value ~ std_length + joint_sex + species, .)), fit_glm = anova(glm(abs(value) ~ std_length + joint_sex + species, ., family = poisson), test = "Chisq"))
...
sl_tab_poiss <- sl_fits %>%
  ...
  ungroup %>%
  select(term, trait, Deviance, Resid..Dev, df, Resid..Df, Pr..Chi.) %>%
  unite(stats, Deviance, Resid..Dev, df, Resid..Df, Pr..Chi.) %>%
  spread(key = term, value = stats)
...
unsl_fits <- pheno_long %>% 
  ...
  .[complete.cases(.),] %>%
  group_by(trait) %>%
  do(fit_lm = anova(lm(value ~ joint_sex + species, .)), fit_glm = anova(glm(abs(value) ~ joint_sex + species, ., family = poisson), test = "Chisq"))
...
write.table(table_stats, "figures/Table1_sex_control.txt", row.names = FALSE, quote = FALSE, sep = "\t")
