...
outlier_wht_cmn_19 <- is.outlier(fx_df_19$fst_wht_cmn, cutoff = 0.99)
outlier_wht_cmn <- c(outlier_wht_cmn, outlier_wht_cmn_19)

outlier_wht_cbr <- is.outlier(fx_df$fst_wht_cbr, cutoff = 0.99)
outlier_wht_cbr_19 <- is.outlier(fx_df_19$fst_wht_cbr, cutoff = 0.99)
outlier_wht_cbr <- c(outlier_wht_cbr, outlier_wht_cbr_19)

outlier_cbr_cmn <- is.outlier(fx_df$fst_cbr_cmn, cutoff = 0.99)
outlier_cbr_cmn_19 <- is.outlier(fx_df_19$fst_cbr_cmn, cutoff = 0.99)
outlier_cbr_cmn <- c(outlier_cbr_cmn, outlier_cbr_cmn_19)

fx_df <- rbind(fx_df, fx_df_19)
...
outlier_names <- c("fst_wht_cmn", "fst_wht_cbr", "fst_cbr_cmn")
...
permute_outlier_windows <- function(index, outlier_df_list, outlier_names = outlier_names, reps = 10000, cutoff = 0.95, fast = TRUE){
  ...
  perm_df <-  mclapply(1:reps, permute_outlier, mc.preschedule = FALSE, mc.set.seed = TRUE, mc.silent = FALSE, 
                    mc.cores = 4, mc.cleanup = TRUE, mc.allow.recursive = TRUE)
  perm_df <- rbind_all(perm_df)
  ...
  if (fast == TRUE){
    ...
    print(paste0("Permuting ", outlier_names[index], "..."))
    
    pval <-  mclapply(emp_df$chr_window, calc_permutation_pval, emp_df = emp_df, perm_df = perm_df, 
                      mc.preschedule = FALSE, mc.set.seed = TRUE, mc.silent = FALSE, 
                      mc.cores = 4, mc.cleanup = TRUE, mc.allow.recursive = TRUE)
