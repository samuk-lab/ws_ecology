...
if [ -f $merged/whtstbk_merged.vcf ];
	then
		echo "$merged/whtstbk_merged.vcf exists, skipping..." 
	else
		echo "Merging filtered files..."
		~/bin/bcftools merge $merged/*.gz --output-type b --merge all > $merged/whtstbk_merged.vcf.gz
fi
...
~/bin/bcftools view \
	...
	--variants-only \
	--types snps \
	whtstbk_merged.vcf.gz \
	--output-type v > whtstbk_merged_bial.vcf

echo "Compressing and indexing..."
bgzip -c whtstbk_merged_bial.vcf > whtstbk_merged.vcf.gz
tabix -p vcf whtstbk_merged.vcf.gz
