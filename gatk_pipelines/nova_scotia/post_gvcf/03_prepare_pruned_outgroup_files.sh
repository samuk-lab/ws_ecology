...
if [ -f subset_vcfs/whtstbk_outgroup_2012.vcf.gz ];
	then
		...
		~/bin/bcftools view --min-alleles 2 --max-alleles 2 --samples $samples --min-af 0.05:minor $vcf --output-type z --types snps > subset_vcfs/whtstbk_outgroup_2014.vcf.gz
		
		echo "All data..."
		samples=$(cat $popfile | awk '{print $1}' | paste -s -d,)
		~/bin/bcftools view --min-alleles 2 --max-alleles 2 --samples $samples --min-af 0.05:minor $vcf --output-type z --types snps > subset_vcfs/whtstbk_outgroup_all.vcf.gz
		
		echo "2012 data only..."
		samples=$(cat $popfile | grep 2012 | awk '{print $1}' | paste -s -d,)
		~/bin/bcftools view --min-alleles 2 --max-alleles 2 --samples $samples --min-af 0.05:minor $vcf --output-type z --types snps > subset_vcfs/whtstbk_outgroup_2012.vcf.gz
fi

sh utils/prepare_pruned_plink_raw.sh subset_vcfs/whtstbk_outgroup_2014.vcf.gz outgroup/whtstbk_outgroup_2014_pruned no_sex
sh utils/prepare_pruned_plink_raw.sh subset_vcfs/whtstbk_outgroup_2012.vcf.gz outgroup/whtstbk_outgroup_2012_pruned no_sex
sh utils/prepare_pruned_plink_raw.sh subset_vcfs/whtstbk_outgroup_all.vcf.gz outgroup/whtstbk_outgroup_all_pruned no_sex
