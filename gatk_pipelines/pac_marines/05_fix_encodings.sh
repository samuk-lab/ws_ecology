...
bam="$proj_home/data/bam_fixed"
bam_rg="$proj_home/data/bam_fixed_rg"
log="$proj_home/log"
...
echo "fixing $prefix..."
echo "cleaning..."
	java -jar $picardtools/CleanSam.jar INPUT=$bam/$prefix.sortrg.bam.fixed.bam OUTPUT=$bam/$prefix.clean.bam 2> $log/$prefix.cleansam.log
	echo "sorting..."
	...
	java -jar $picardtools/AddOrReplaceReadGroups.jar I=$bam/$prefix.sort.bam O= $bam_rg/$prefix.sortrg.bam SORT_ORDER=coordinate RGID=$prefix RGLB=$project RGPL=ILLUMINA RGPU=$project RGSM=$prefix CREATE_INDEX=True 2> $log/$prefix.addRG.log
